## ============================================================
## scripts/run_n1500_headline_seed1.R
##
## JOB 1 of the four-job Hellbender batch — the polished "headline"
## n=1500 run on seed=1 with a longer chain (n_iter=6000) and a
## widened sigma2 MH proposal to chase the low accept rate seen on
## the original n=1500 run (accept ≈ 0.10 with mh_sd_log_sigma2=0.5).
##
## Settings vs run_n1500_allpairs.R:
##   - n_iter:  3000 -> 6000  (longer chain, post-thin yields 2000 draws)
##   - n_thin:  1    -> 2
##   - mh_sd_log_sigma2: 0.5 -> 0.3 (tighten proposal — previous 0.5 with
##                                   accept 0.10 suggests the proposal is
##                                   too wide for this posterior; halving
##                                   the SD targets ≈ 0.3 accept)
##   - mh_sd_log_rho:  0.15   (same as previous tuned value)
##   - mh_sd_log_tau2: 0.6    (same as previous tuned value)
##
## Output: output/fit_medsat_n1500_seed1_headline_hb.rds
##
## Usage on Hellbender (submitted by .sub):
##   Rscript scripts/run_n1500_headline_seed1.R
##
## Local dry-run (tiny, discards output):
##   MEDSAT_DRY_RUN=1 Rscript scripts/run_n1500_headline_seed1.R
## ============================================================

IS_DRY_RUN <- identical(Sys.getenv("MEDSAT_DRY_RUN"), "1")

cat("============================================================\n")
cat(sprintf(" run_n1500_headline_seed1.R  |  %s\n", Sys.time()))
...
if (IS_DRY_RUN) {
  ...
  OUT_FIT <- file.path(tempdir(),
                       "fit_medsat_n1500_seed1_headline_DRYRUN.rds")
} else {
  settings <- list(
    M                = 20L,
    nu               = 1.0,
  ...
  OUT_FIT <- "output/fit_medsat_n1500_seed1_headline_hb.rds"
}
...
fit$reproducibility <- list(
  ...
  hostname    = host,
  wd          = getwd(),
  script_path = "scripts/run_n1500_headline_seed1.R",
