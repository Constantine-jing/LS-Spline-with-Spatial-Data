...
#                                   real-data sim objects supported)
#
...
#   - May 2026: tolerates real-data sim objects (Option A). If
#     sim$truth_params$rho / sigma2 / tau2_s are NA, the REML init
#     falls back to data-driven defaults (rho_init = 0.2,
#     sigma2_init = tau2_s_init = var(y)/2). `nu` is still required
#     (it's a fixed modelling choice, not a truth quantity).
#
...
fit_ours_interaction <- function(sim, settings = NULL) {
  ...
  if (S$fit_interactions && length(sim$int_keys) == 0L) {
    stop("fit_interactions = TRUE but sim has no interaction keys.")
  }
  ...
  # ----- Option A: real-data sim objects may have NA truth_params -----
  # Use data-driven defaults for the REML init in that case. These only
  # affect MCMC starting values; they are not priors and not "truth".
  ...
  lambda_init_val <- sigma2_init_val / max(tau2_s_init_val, 0.01)
  if (S$verbose) {
    cat(sprintf("[fit_ours_interaction] REML init: rho=%.3f  lambda=%.3f\n",
                rho_init_val, lambda_init_val))
  }

  obj6 <- fit_ls_spatial(y = y, X_raw = X_raw, coords = coords,
                         M_vec = rep(6, sim$p), nu = S$nu,
                         rho_init    = rho_init_val,
                         lambda_init = lambda_init_val,
                         verbose = FALSE)
