...
pdf("bayes_out/sim3_n1000_reml_marginals.pdf", width = 10, height = 6)
par(mfrow = c(2, 2))
plot_marginals(reml_curves, var_names = var_names,
               show_rmse = TRUE, ylim_shared = FALSE)
dev.off()
cat("  Plot: bayes_out/sim3_n1000_reml_marginals.pdf\n")
...
pdf("bayes_out/sim3_n1000_bayes_marginals.pdf", width = 10, height = 6)
par(mfrow = c(2, 2))
plot_gibbs_marginals(gs, obj_bayes, x_grid = x_grid,
                     truth_f_list = truth_f_list_4,
                     var_names = var_names, X_raw = X_raw)
dev.off()
cat("  Plot: bayes_out/sim3_n1000_bayes_marginals.pdf\n")

pdf("bayes_out/sim3_n1000_bayes_trace.pdf", width = 10, height = 10)
plot_gibbs_trace_full(gs, true_sigma2 = sigma2_true,
                          true_tau2   = tau2_true,
                          true_rho    = rho_true)
dev.off()
cat("  Plot: bayes_out/sim3_n1000_bayes_trace.pdf\n")
...
write.csv(comp_table, "bayes_out/sim3_n1000_reml_vs_bayes.csv", row.names = FALSE)
write.csv(cov_tab,   "bayes_out/sim3_n1000_bayes_coverage.csv",  row.names = FALSE)

cat("\nOutputs in bayes_out/:\n")
cat("  sim3_n1000_reml_marginals.pdf\n")
cat("  sim3_n1000_bayes_marginals.pdf\n")
