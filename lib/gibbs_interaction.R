...
gibbs_interaction_sampler <- function(
    ...
    jitter    = 1e-8,
    verbose   = TRUE,
    # ---- DIAGNOSTIC HOOK (added May 2026 for Hypothesis 3 confirmation) ----
    # Optional numeric vector of length n_main. If element j is finite (not NA
    # and not NULL element), the IG draw for tau2_s_main[j] in Step 5 is
    # SKIPPED and the value is held fixed at the supplied number. NA entries
    # (or NULL argument) leave the conjugate IG update untouched.
    # Used to test whether undersmoothing on f_j is the cause of poor RMSE:
    # set tau2_s_main_fixed = c(0.005, NA, NA) to clamp f_1's smoothing
    # variance and see if the wiggle disappears.
    tau2_s_main_fixed = NULL,

    # ---- DIAGNOSTIC HOOK (added May 2026 for plan B confirmation) ----
    # Same idea as tau2_s_main_fixed but for interaction blocks. Length
    # equals n_int (number of interaction pairs); finite entries are
    # clamped, NA entries get conjugate IG draws.
    tau2_s_int_fixed = NULL,

    # ---- DIAGNOSTIC HOOK (added May 2026 for sigma-clamp test) ----
    # Optional scalar clamps for the variance components. When finite,
    # the MH step for that component is SKIPPED and the value held fixed.
    # Used to test whether per-iteration Sigma fluctuation is the source
    # of the GLS-vs-MCMC bias gap on f_1.
    ...
    # The caller is responsible for passing an empty col_map_int /
    # K_int_list (and not including interaction columns in H) so that
    # the rest of the sampler runs an additive model. Asserted below.
  ...
  for (iter in seq_len(n_iter)) {
    ...
    for (j in seq_len(n_main)) {
      # ---- DIAGNOSTIC HOOK: skip the IG draw if j-th component is clamped ----
    ...
    # Gated on fit_interactions: when FALSE the loop body is skipped
    # (and n_int is asserted to be zero above, so this is belt-and-
    # suspenders).
    # ----------------------------------------------------------
    if (fit_interactions) {
      for (k in seq_len(n_int)) {
        # ---- DIAGNOSTIC HOOK: skip the IG draw if k-th component is clamped ----
