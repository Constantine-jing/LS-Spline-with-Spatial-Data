...
# NOTE: REML marginal plot deferred until after Bayes, so we
#       can compute a shared y-axis across both methods.
...
# ============================================================
# Shared y-axis: compute ylim across REML curves + Bayes bands
# ============================================================
# Pre-compute Bayes bands for ylim calculation
...
# Gather all y-values from both methods
...
a <- max(abs(shared_rng))            # symmetric
shared_rng <- c(-a, a)
span <- diff(shared_rng)
if (span == 0) span <- 1
shared_ylim <- shared_rng + c(-1, 1) * 0.05 * span

cat(sprintf("  Shared ylim: [%.3f, %.3f]\n", shared_ylim[1], shared_ylim[2]))

# --- REML marginals (shared y-axis) ---
pdf("bayes_test/sim1_n1000_reml_marginals.pdf", width = 10, height = 6)
par(mfrow = c(2, 2))
plot_marginals(reml_curves, var_names = var_names,
               show_rmse = TRUE, ylim_shared = FALSE, ylim = shared_ylim)
dev.off()
cat("  Plot: bayes_test/sim1_n1000_reml_marginals.pdf\n")

# --- Bayes marginals (shared y-axis) ---
# Use pre-computed bands; plot inline to honour shared_ylim
pdf("bayes_test/sim1_n1000_bayes_marginals.pdf", width = 10, height = 6)
par(mfrow = c(2, 2))
for (j in 1:p) {
  b <- bayes_bands[[j]]
  ...
  legend("topleft",
         legend = c("posterior mean", "95% band",
...
dev.off()
cat("  Plot: bayes_test/sim1_n1000_bayes_marginals.pdf\n")
...
write.csv(comp_table, "bayes_test/sim1_n1000_plot_new_reml_vs_bayes.csv", row.names = FALSE)
write.csv(cov_tab,   "bayes_test/sim1_n1000_plot_new_bayes_coverage.csv",  row.names = FALSE)

cat("\nOutputs in bayes_test/:\n")
cat("  sim1_n1000_plot_new_reml_marginals.pdf\n")
cat("  sim1_n1000_plot_new_bayes_marginals.pdf\n")
cat("  sim1_n1000_plot_new_bayes_trace.pdf\n")
cat("  sim1_n1000_plot_new_reml_vs_bayes.csv\n")
cat("  sim1_n1000_plot_new_bayes_coverage.csv\n")
