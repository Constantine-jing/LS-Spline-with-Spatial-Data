# ============================================================
# run_sim2_n1000.R
# Sim2: Spatial effect present, iid covariates (p=4)
#   n = 1000, M_Bayes = 20, M_REML = 6
#   Full Bayes (rho sampled, RW2) vs REML
#
# Key check: sigma2 should recover toward 0.8, b posterior
# should correlate with b_true. Curve recovery benefits from
# Bayes vs REML -- clean setting with no confounding.
#
# True parameters:
#   sigma2=0.8, tau2=0.15, rho=0.2
#   f1=2sin(pi*x), f2=1.5exp(x-0.5), f3=0.7x^2, f4=0.5sin(2pi*x)
#   X1-X4: iid Uniform(0,1)
#
# MH proposals (tuned):
...
# ============================================================
# Simulation: Sim2
#   X1-X4: iid Uniform(0,1)  -- no spatial correlation in X
#   b:     Matern spatial random effect, IS added to Y
#   Y = eta(X) + b + eps
# ============================================================
simulate_sim2 <- function(n = 1000, p = 4, mu = 0,
...
# Generate data
# ============================================================
cat(sprintf("\n=== Sim2: n=%d, p=%d (iid X, WITH spatial b) ===\n", n, p))
dat    <- simulate_sim2(n = n, p = p, sigma2 = sigma2_true,
...
pdf("bayes_out/sim2_n1000_reml_marginals.pdf", width = 10, height = 6)
par(mfrow = c(2, 2))
plot_marginals(reml_curves, var_names = var_names,
               show_rmse = TRUE, ylim_shared = FALSE)
dev.off()
cat("  Plot: bayes_out/sim2_n1000_reml_marginals.pdf\n")
...
pdf("bayes_out/sim2_n1000_bayes_marginals.pdf", width = 10, height = 6)
par(mfrow = c(2, 2))
plot_gibbs_marginals(gs, obj_bayes, x_grid = x_grid,
                     truth_f_list = truth_f_list_4,
                     var_names = var_names, X_raw = X_raw)
dev.off()
cat("  Plot: bayes_out/sim2_n1000_bayes_marginals.pdf\n")

pdf("bayes_out/sim2_n1000_bayes_trace.pdf", width = 10, height = 10)
plot_gibbs_trace_full(gs, true_sigma2 = sigma2_true,
                          true_tau2   = tau2_true,
                          true_rho    = rho_true)
dev.off()
cat("  Plot: bayes_out/sim2_n1000_bayes_trace.pdf\n")

# ============================================================
# Summary + CSVs
# ============================================================
cat("\n\n####################################################\n")
cat("# REML vs Bayes: Sim2 n=1000\n")
cat("####################################################\n\n")
...
write.csv(comp_table, "bayes_out/sim2_n1000_reml_vs_bayes.csv", row.names = FALSE)
write.csv(cov_tab,   "bayes_out/sim2_n1000_bayes_coverage.csv",  row.names = FALSE)

cat("\nOutputs in bayes_out/:\n")
cat("  sim2_n1000_reml_marginals.pdf\n")
cat("  sim2_n1000_bayes_marginals.pdf\n")
cat("  sim2_n1000_bayes_trace.pdf\n")
cat("  sim2_n1000_reml_vs_bayes.csv\n")
cat("  sim2_n1000_bayes_coverage.csv\n")
